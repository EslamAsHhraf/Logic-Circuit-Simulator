...
pincoor OR2::getouppcoor()
{
	return outp;
}

pincoor* OR2::getinppcoor()
{
	return inpp;
}
OutputPin& OR2::getm_outputpin()
{
	return m_OutputPin;
}
////////////////

void OR2::setinpconn(int n, bool valid)
{
	inpconn[n] = valid;
}
...
InputPin* OR2::getm_InpPutpin(int n)
{
	return &m_InputPins[n];
}
void OR2::DoAllConnections()
{
	m_OutputPin.getAllConnection();
}

OR2::OR2(const OR2& C) :Gate(2, AND2_FANOUT)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void OR2::Get_String(Input* pIn,Output* pOut)
{
	
	set_string(pIn->GetSrting(pOut,m_GfxInfo));
}

void OR2:: SAVE(ofstream &Infofile,int e)
{
	if(e==0)
	Infofile<<"OR2\t"<<ID<<"\t"<<getLabel()<<"\t"<<(m_GfxInfo.x1)<<"\t"<<(m_GfxInfo.y1)<<"\n";//no tab after"AND2"
}
...
void OR2::ComponentLow(Output * POut)
{
	m_OutputPin.setStatus(LOW);
}