...
pincoor NOR3::getouppcoor()
{
	return outp;
}

pincoor* NOR3::getinppcoor()
{
	return inpp;
}
OutputPin& NOR3::getm_outputpin()
{
	return m_OutputPin;
}
////////////////

void NOR3::setinpconn(int n, bool valid)
{
	inpconn[n] = valid;
}
...
InputPin* NOR3::getm_InpPutpin(int n)
{
	return &m_InputPins[n];
}
void NOR3::DoAllConnections()
{
	m_OutputPin.getAllConnection();
}

NOR3::NOR3(const NOR3& C) :Gate(3, AND2_FANOUT)
{
	//currentinpp = 0;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void NOR3::Get_String(Input* pIn,Output* pOut)
{
	
	set_string(pIn->GetSrting(pOut,m_GfxInfo));
}

void NOR3:: SAVE(ofstream &Infofile,int e)
{
	if(e==0)
	Infofile<<"NOR3\t"<<ID<<"\t"<<getLabel()<<"\t"<<(m_GfxInfo.x1)<<"\t"<<(m_GfxInfo.y1)<<"\n";//no tab after"AND2"
}
...
int NOR3:: get_ID()
 ...
 void NOR3::set_string(string t)
 {
	 Component::set_string(t);
 }
string NOR3::getLabel()
{
	return (Component::getLabel());
}
void NOR3::SetCompCoordinates(GraphicsInfo Coordinate)
{
...
void NOR3::ComponentLow(Output * POut)
{
	m_OutputPin.setStatus(LOW);
}