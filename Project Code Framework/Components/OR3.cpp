...
void OR3::Draw(Output* pOut, bool select)
{
	//Call output class and pass gate drawing info to it.
	pOut->DrawOR3(m_GfxInfo, select);
}
...
pincoor OR3::getouppcoor()
{
	return outp;
}

pincoor* OR3::getinppcoor()
{
	return inpp;
}
OutputPin& OR3::getm_outputpin()
{
	return m_OutputPin;
}
////////////////

void OR3::setinpconn(int n, bool valid)
{
	inpconn[n] = valid;
}
...
InputPin* OR3::getm_InpPutpin(int n)
{
	return &m_InputPins[n];
}
void OR3::DoAllConnections()
{
	m_OutputPin.getAllConnection();
}

OR3::OR3(const OR3& C) :Gate(3, AND2_FANOUT)
{
	//currentinpp = 0;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void OR3::Get_String(Input* pIn,Output* pOut)
{
	
	set_string(pIn->GetSrting(pOut,m_GfxInfo));
}

void OR3:: SAVE(ofstream &Infofile,int e)
{
	if(e==0)
	Infofile<<"OR3\t"<<ID<<"\t"<<getLabel()<<"\t"<<(m_GfxInfo.x1)<<"\t"<<(m_GfxInfo.y1)<<"\n";//no tab after"AND2"
}

void OR3:: setID(int id)
{
	ID=id;
}
int OR3:: get_ID()
...
void OR3::SetCompCoordinates(GraphicsInfo Coordinate)
{
...
void OR3::ComponentLow(Output * POut)
{
	m_OutputPin.setStatus(LOW);
}